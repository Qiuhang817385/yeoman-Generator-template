...
<body>
        ...
        <script>
            /* 
                需求,把login这样的url,用户看起来不舒服,希望改成mylogin,
                1,改routes,规则里面的path
                2.改router-link里面的
            ...
            var router = new VueRouter({
                  //4.配置路由对象,登陆注册两个路由,
                //   路由对象有了名称就等于有了变量名,router-link只需要说明这个变量名就可以了
                routes:[
                    { name:'login1',path:'/login',component:Login},
                    { name:'regin1',path:'/regin',component:Regin} ]
            ...
            var App = {
                template:`
                    ...
                    <div>
                       <!-- vue-router内置组件router-link,不需要再写#的问题
                        ...
                        <router-link :to="{name:'regin1'}">注册</router-link>
                        <!--留坑-->
